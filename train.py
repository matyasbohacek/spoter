...
def get_default_args():
    ...
    parser.add_argument("--experiment_name", type=str, default="lsa_64_spot", help="Name of the experiment after which the logs and plots will be named")
    ...
    parser.add_argument("--seed", type=int, default=379, help="Seed with which to initialize all the random components of the training")

    # Data
    parser.add_argument("--training_set", type=str, default="/storage/plzen4-ntis/projects/cv/spot/LSA64_60fps.csv", help="Path to the training dataset CSV file")
    parser.add_argument("--validation_set", type=str, default="", help="Path to the validation dataset CSV file")
    parser.add_argument("--testing_set", type=str, default="", help="Path to the testing dataset CSV file")
    ...
    parser.add_argument("--scheduler_patience", type=int, default=5, help="Patience for the ReduceLROnPlateau scheduler")
...
def train(args):
    ...
    # vision_tt = VisionTransformer(dim=2, mlp_dim=108, num_classes=100, depth=12, heads=8)
    ...
    train_set = CzechSLRDataset(args.training_set, transform=transform, augmentations=True)
    ...
    if args.validation_set:
        #val_set = CzechSLRDataset(args.validation_set)
        #val_loader = DataLoader(val_set, shuffle=True, generator=g)
        pass
    ...
    # Testing set
    if args.testing_set:
        eval_set = CzechSLRDataset(args.testing_set)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0:
            ...
            # TEMP
            # slrt_model.train(False)
            # _, _, eval_acc = evaluate(slrt_model, eval_loader, device)
            # slrt_model.train(True)
    ...
    # PLOTTING 0: Performance (loss, accuracies) chart plotting
    if args.plot_stats:
        fig, ax = plt.subplots()
    ...
    # PLOTTING 1: Learning rate progress
    if args.plot_lr:
